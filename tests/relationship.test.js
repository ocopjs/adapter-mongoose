const { relationshipTokenizer } = require("../src/lib/tokenizers");
...
describe("Relationship tokenizer", () => {
  test("Uses correct conditions", () => {
    const relationshipConditions = {
      isRelationship: true,
      getListByKey: () => ({
        adapter: { key: "Bar", model: { collection: { name: "name" } } },
      }),
      field: { many: false },
      path: "name",
      rel: {},
    ...
    expect(
      relationshipTokenizer(listAdapter, "name", ["name"], () => "abc123"),
    ).toMatchObject({
  ...
  test("returns empty array when no matches found", () => {
    ...
    const result = relationshipTokenizer(
      listAdapter,
      "name",
      ["name"],
      () => {},
