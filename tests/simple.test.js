const { simpleTokenizer, modifierTokenizer } = require("../src/lib/tokenizers");
...
describe("Simple tokenizer", () => {
  test("Uses correct conditions", () => {
    ...
    expect(
      simpleTokenizer(listAdapter, { name: "hi" }, "name", ["name"]),
    ).toMatchObject({
      foo: "bar",
    });
  ...
  test("Falls back to modifier conditions when no simple condition found", () => {
    ...
    expect(
      modifierTokenizer(listAdapter, { $count: "hi" }, "$count", ["$count"]),
    ).toMatchObject({
      $count: "hi",
    });
  ...
  test("returns empty array when no matches found", () => {
    ...
    const result = simpleTokenizer(listAdapter, { name: "hi" }, "name", [
      "name",
    ]);
