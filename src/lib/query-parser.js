const { getType, flatten } = require("@ocopjs/utils");
...
const flattenQueries = (parsedQueries, joinOp) => ({
  ...
  postJoinPipeline: flatten(
    parsedQueries.map((q) => q.postJoinPipeline || []),
  ).filter((pipe) => pipe),
...
function queryParser({ listAdapter, getUID }, query, pathSoFar = [], include) {
  if (getType(query) !== "Object") {
    throw new Error(
      `Expected an Object for query, got ${getType(
        query,
      )} at path ${pathSoFar.join(".")}`,
    );
  }
  ...
  const parsedQueries = Object.entries(query).map(([key, value]) => {
    ...
    if (["AND", "OR"].includes(key)) {
      ...
      ["$search", "$sortBy", "$orderBy", "$skip", "$first", "$count"].includes(
        key,
      )
    ) {
      return {
        postJoinPipeline: [modifierTokenizer(listAdapter, query, key, path)],
      };
    } else if (key === "id") {
      ...
      return {
        ...
        relationships: [
          {
            relationshipInfo,
            ...queryParser({ listAdapter, getUID }, value, path),
          },
        ],
      };
    } else {
  ...
  return {
    ...flatQueries,
    excludeFields: excludeFields.filter(
      (field) => !includeFields.includes(field),
    ),
  };
}
