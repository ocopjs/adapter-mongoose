...
class MongooseAdapter extends BaseOcopAdapter {
  ...
  async postConnect({ rels }) {
    ...
    Object.values(this.listAdapters).forEach((listAdapter) => {
      listAdapter.fieldAdapters.forEach((fieldAdapter) => {
        fieldAdapter.addToMongooseSchema(
          listAdapter.schema,
          listAdapter.mongoose,
          rels,
        );
      });
    });
  ...
  getDefaultPrimaryKeyConfig() {
    ...
    throw new Error(
      "in your schema, config field `id: { type: MongoId }` to using mongoose adapter!",
    );
  }
  ...
  async checkDatabaseVersion() {
    ...
    try {
      info = await new this.mongoose.mongo.Admin(
...
class MongooseListAdapter extends BaseListAdapter {
  ...
  async _postConnect({ rels }) {
    ...
    this.fieldAdapters.forEach((fieldAdapter) => {
      fieldAdapter.rel = rels.find(
        ({ left, right }) =>
          left.adapter === fieldAdapter ||
          (right && right.adapter === fieldAdapter),
      );
      if (fieldAdapter._hasRealKeys()) {
        this.realKeys.push(
          ...(fieldAdapter.realKeys
            ? fieldAdapter.realKeys
            : [fieldAdapter.path]),
        );
      }
  ...
  async _unsetOneToOneValues(realData) {
    ...
    await Promise.all(
      Object.entries(realData)
        .map(([key, value]) => ({
          value,
          adapter: this.fieldAdaptersByPath[key],
        }))
        .filter(({ adapter }) => adapter && adapter.isRelationship)
        .filter(
          ({ value, adapter: { rel } }) =>
            rel.cardinality === "1:1" &&
            rel.tableName === this.key &&
            value !== null,
        )
        .map(
          ({
  ...
  async _unsetForeignOneToOneValues(data, id) {
    ...
    await Promise.all(
      Object.keys(data)
        ...
        .filter(
          ({ adapter: { rel } }) =>
            rel.cardinality === "1:1" && rel.tableName !== this.key,
        )
        .map(
          ({
            adapter: {
  ...
  async _create(data) {
    ...
    const manyItem = await this._processNonRealFields(
      data,
      async ({ value, adapter }) =>
  ...
  async _update(id, data) {
    ...
    await this._processNonRealFields(
      data,
      async ({ path, value: newValues, adapter }) => {
        const { cardinality, columnName, tableName } = adapter.rel;
        let value;
        ...
        if (cardinality !== "1:1") {
          ...
          const needsDelete = currentRefIds.filter(
          ...
          if (newValues === null) {
            ...
            await this._setNullByValue({
              tableName,
              columnName: selectCol,
              value: item.id,
            });
          }
          value = newValues;
    ...
    return (
      (await this._itemsQuery({ where: { id: item.id }, first: 1 }))[0] || null
    );
  }
  ...
  graphQlQueryPathToMongoField(path) {
    ...
    if (!fieldAdapter) {
      throw new Error(
        `Unable to find Mongo field which maps to graphQL path ${path}`,
      );
    }
...
class MongooseFieldAdapter extends BaseFieldAdapter {
  ...
  _hasRealKeys() {
    ...
    return !(
      this.isRelationship &&
      (this.config.many ||
        (this.rel.cardinality === "1:1" && this.rel.right.adapter === this))
    );
  }
  ...
  equalityConditionsInsensitive(dbPath, f = identity) {
    return {
      [`${this.path}_i`]: (value) => ({
        [dbPath]: new RegExp(`^${escapeRegExp(f(value))}$`, "i"),
      }),
  ...
  inConditions(dbPath, f = identity) {
    return {
      [`${this.path}_in`]: (value) => ({
        [dbPath]: { $in: value.map((s) => f(s)) },
      }),
      [`${this.path}_not_in`]: (value) => ({
        [dbPath]: { $not: { $in: value.map((s) => f(s)) } },
      }),
    };
  }
