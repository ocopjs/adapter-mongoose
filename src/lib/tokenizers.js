...
const getRelatedListAdapterFromQueryPath = (listAdapter, queryPath) => {
  ...
  for (let index = 0; index < queryPath.length; index++) {
    ...
    const fieldAdapter = foundListAdapter.fieldAdapters
      ...
      .find(({ path }) =>
        [path, `${path}_every`, `${path}_some`, `${path}_none`].includes(
          segment,
        ),
      );
    ...
    foundListAdapter = fieldAdapter.getListByKey(
      fieldAdapter.refListKey,
    ).adapter;
...
const relationshipTokenizer = (
  ...
  const refListAdapter2 = fieldAdapter.getListByKey(
    fieldAdapter.refListKey,
  ).adapter;
...
const modifierTokenizer = (listAdapter, query, queryKey, path) => {
  ...
  return {
    ...
    $search: (value) => {
      ...
      // return {
      //   $match: {
      //     [searchFieldName]: new RegExp(`${escapeRegExp(value)}`, "i"),
      //   },
      // };
    },
